...
Drizzle is a performant database toolkit that makes it easy to access databases with type safety and auto-completion. It is used to define the data model and generate the database schema. All of the database-related code is within the [/packages/db](https://github.com/sandboxnu/cooper/tree/main/packages/db) folder. All of the database schemas are defined in the `/schema` folder.
